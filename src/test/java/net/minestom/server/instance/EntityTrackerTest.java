...
public class EntityTrackerTest {
    ...
    public void register() {
        ...
        assertTrue(tracker.selectGlobalEntity(EntitySelector.selector(builder -> builder.chunk(Vec.ZERO))).toList().isEmpty());

        tracker.register(ent1, Vec.ZERO, updater);
        assertEquals(1, tracker.selectGlobalEntity(EntitySelector.selector(builder -> builder.chunk(Vec.ZERO))).toList().size());

        tracker.unregister(ent1, updater);
        assertEquals(0, tracker.selectGlobalEntity(EntitySelector.selector(builder -> builder.chunk(Vec.ZERO))).toList().size());
    }
    ...
    public void move() {
        ...
        assertEquals(0, tracker.selectGlobalEntity(EntitySelector.selector(builder -> builder.chunk(Vec.ZERO))).toList().size());
        assertEquals(1, tracker.selectGlobalEntity(EntitySelector.selector(builder -> builder.chunk(new Vec(32, 0, 32)))).toList().size());
    }
